...
def main(data, dataset, granularity, train, test, prd_trn, prd_tst, index_col, target):
    plot_evaluation_results(train.values, prd_trn, test.values, prd_tst, 'persistence_eval')
    savefig(f'images/time_series/persistance_model/'+dataset+'/persistence_eval'+granularity+'.png')

    plot_forecasting_series(train, test, prd_trn, prd_tst, 'persistence_plots',
                            x_label=index_col,
                            y_label=target)
...
main(data, 'dataset1','Daily', train, test, prd_trn, prd_tst, 'Date', 'Glucose')
