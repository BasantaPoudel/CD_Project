...
data = read_csv(filename, na_values='-1', index_col='encounter_id',  parse_dates=True, infer_datetime_format=True)
...
if 'patient_nbr' in df:
    df = df.drop("patient_nbr", axis='columns') '''
...
df.to_csv(f'data/classification/datasets_for_further_analysis/dataset1/{file}_dropna_drop_columns_mv.csv', index=True)
